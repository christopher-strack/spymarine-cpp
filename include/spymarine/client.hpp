...
public:
  ...
  request_devices() noexcept {
    return request_count_info().and_then(
        [this](
            const count_info i) -> std::expected<std::vector<device2>, error> {
          std::vector<device2> infos;
          infos.reserve(i.device_count);

          for (const auto id : std::views::iota(uint8_t(0), i.device_count)) {
            std::expected<device2, error> info = request_device(id);
...
private:
  ...
  write_message_data(const message_type type,
    ...
    const auto cb = std::bit_cast<std::array<uint8_t, 2>>(
        crc(buffer.subspan(1, payload_size - 2)));
    const auto copy_crc =
