...
constexpr device_id get_device_id(const device2& device) noexcept {
  return std::visit([](const auto& dev) { return dev.id; }, device);
}

constexpr std::optional<std::string>
get_device_name(const device2& device) noexcept {
  return std::visit([](const auto& dev) { return dev.name; }, device);
}
